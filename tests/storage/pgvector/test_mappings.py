"""tsquery sanitization, RRF fusion, and payload round-trip — pure, always run."""
import pytest

from ingestlib.operations.split.models import Chunk
from ingestlib.storage.pgvector.store import (
    _breadcrumb,
    _filter_sql,
    _from_payload,
    _rrf,
...
def test_ts_query_ors_word_tokens():
    assert _ts_query("recruited Cairo centers") == "recruited | Cairo | centers"


def test_ts_query_survives_query_syntax():
    # raw tsquery operators and punctuation must come out as harmless tokens
    assert _ts_query("what's the +360% growth?") == "what | s | the | 360 | growth"
    assert _ts_query("a & b | !c <-> (d)") == "a | b | c | d"


def test_ts_query_none_when_nothing_tokenizes():
    assert _ts_query("?!… — 🚀") is None
    assert _ts_query("   ") is None


def test_rrf_boosts_agreement_and_keeps_dense_on_ties():
    fused = _rrf([1, 2, 3], [3, 4])
    order = [row_id for row_id, _ in fused]
    assert order[0] == 3, "row on both lists must outrank any single-list row"
...
def test_filter_sql_builds_equality():
    sql, params = _filter_sql({"category": "invoice", "kind": "table"})
    assert sql == " AND category = %s AND kind = %s"
