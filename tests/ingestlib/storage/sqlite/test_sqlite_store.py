...
def test_count_vectors_reports_the_live_total(store, upserted):
    """The durability-ledger method verify() audits against (registry
    chunk_count vs what the store actually holds)."""
    assert store.count_vectors(_DOC_ID) == 2
    assert store.count_vectors(_DOC_ID, namespace="prod") == 0  # namespace-scoped
    assert store.count_vectors("never-stored") == 0


def test_count_vectors_is_zero_on_a_fresh_database(store):
    assert store.count_vectors(_DOC_ID) == 0
