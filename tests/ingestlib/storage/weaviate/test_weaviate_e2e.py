...
native BM25), queries dense, hybrid-fused, and filtered, then deletes it.
"""
import os
...
def store():
    from ingestlib.storage import WeaviateStore
...
def test_upsert_writes_all_objects(upserted):
    assert upserted == 2


def test_count_vectors_reports_the_live_total(store, upserted):
    """The durability-ledger method verify() audits against (aggregate count)."""
...
def test_reupsert_overwrites_not_duplicates(store, upserted):
    ...
    assert len(ours) == 2, "deterministic object IDs must overwrite, never duplicate"
...
def test_reupsert_with_fewer_chunks_leaves_no_orphans(store, upserted):
    ...
    chunk_ids behind as stale objects."""
    from ingestlib.foundations.llm import embed_text
...
def test_namespaces_are_isolated(store, upserted):
    """The same document in two namespaces must not collide — object IDs are
    namespace-scoped and queries filter on the namespace property."""
    from ingestlib.foundations.llm import embed_text
...
def test_delete_document_removes_objects(store, upserted):
