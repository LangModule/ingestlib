"""Full connector round-trip against a real Milvus server.
...
pytestmark = pytest.mark.skipif(
    os.environ.get("RUN_MILVUS_E2E") != "1",
    reason="milvus e2e is opt-in: set RUN_MILVUS_E2E=1 (needs a Milvus server at MILVUS_URL)",
)

_DOC_ID = "test-milvus-doc"
...
def store():
    ...
    from ingestlib.config import MilvusConfig, get_config
    from ingestlib.storage import MilvusStore
    from ingestlib.storage.milvus.client import get_milvus_client, reset_milvus_client

    cfg = get_config()
    patched = dataclasses.replace(
        cfg,
        milvus=MilvusConfig(url=cfg.milvus.url, token=cfg.milvus.token,
...
def test_count_vectors_reports_the_live_total(store, upserted):
    """The durability-ledger method verify() audits against."""
    assert store.count_vectors(_DOC_ID) == 2
    assert store.count_vectors("never-stored") == 0
...
def test_hybrid_fusion_surfaces_lexical_match(store, upserted):
    """Server-side RRF: exact tokens must surface the right chunk even when
...
def test_analyzer_stemming_matches_inflected_query(store, upserted):
...
def test_punctuation_heavy_text_is_a_search_not_an_error(store, upserted):
    # BM25 search takes raw text through the analyzer — nothing to sanitize
    hits = store.query(_NEAR_RECRUITMENT, top_k=2, text="what's the +360% growth?")
    assert hits
    hits = store.query(_NEAR_RECRUITMENT, top_k=2, text="?!… — 🚀")
    assert hits, "untokenizable text still returns dense results"


def test_filter_value_with_quotes_cannot_escape_expression(store, upserted):
    hits = store.query(
        _NEAR_RECRUITMENT, top_k=5,
        filters={"section": 'nope" or namespace != "'},
    )
    assert hits == [], "a quote-laden value must stay a literal, never widen the filter"
...
def test_delete_document_removes_all_traces(store, upserted):
    ...
    assert store.query(_NEAR_RECRUITMENT, top_k=5, text="recruited Cairo") == [], (
        "the BM25 side must vanish with the rows"
    )
