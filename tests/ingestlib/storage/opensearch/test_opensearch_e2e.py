...
server). Creates the real index on first run, upserts a sentinel document
(dense + BM25 text), queries dense, hybrid-fused, and filtered, then
deletes it.
"""
import os
...
def store():
    from ingestlib.storage import OpensearchStore
...
def test_upsert_writes_all_documents(upserted):
    assert upserted == 2
...
def test_reupsert_overwrites_not_duplicates(store, upserted):
    ...
    assert len(ours) == 2, "deterministic _ids must overwrite, never duplicate"
...
def test_reupsert_with_fewer_chunks_leaves_no_orphans(store, upserted):
    ...
    chunk_ids behind as stale rows."""
    from ingestlib.foundations.llm import embed_text
...
def test_namespaces_are_isolated(store, upserted):
    """The same document in two namespaces must not collide — _ids are
    namespace-scoped and queries filter on the namespace field."""
    from ingestlib.foundations.llm import embed_text
...
def test_delete_document_removes_rows(store, upserted):
