"""reindex() — a vector store rebuilt from the registry. Always run:
embedding is stubbed at the ingestor seam (reindex reuses _embed_chunks),
the registry + sqlite are real, so the rebuild path is proven end to end.
"""
import dataclasses

import pytest

import ingestlib.config as config_module
from ingestlib.config import SqliteConfig, get_config
from ingestlib.services import reindex
from ingestlib.storage import SqliteStore

from tests.ingestlib.services.conftest import vec

_DOC_A = "reindex-a-" + "0" * 54
_DOC_B = "reindex-b-" + "1" * 54


@pytest.fixture()
def stub_embeddings(monkeypatch):
    """reindex embeds through the ingestor's _embed_chunks → aembed_text."""
    import importlib

    ingestor = importlib.import_module("ingestlib.services.ingest.ingestor")

    async def fake_embed(text, purpose="GENERIC_INDEX", dimension=1024):
        return vec(1.0)

    monkeypatch.setattr(ingestor, "aembed_text", fake_embed)


def test_reindex_rebuilds_a_wiped_store(stack, make_document, stub_embeddings,
                                        tmp_path, monkeypatch):
    """The provider-switch / new-store scenario: the registry has the chunks, the
    target store is empty — reindex fills it without any pipeline run."""
    make_document(_DOC_A, stack.corpus / "a.pdf")
    make_document(_DOC_B, stack.corpus / "b.pdf")

    # a FRESH store standing in for "the new connector"
    cfg = dataclasses.replace(
        get_config(), sqlite=SqliteConfig(path=tmp_path / "fresh.db")
    )
    monkeypatch.setattr(config_module, "_config", cfg)
    fresh = SqliteStore()
    assert fresh.query(vec(1.0), top_k=5) == []

    result = reindex(store=fresh)

    assert result.documents == 2
    assert result.chunks == 2
    assert result.skipped == []
    hits = fresh.query(vec(1.0), top_k=5)
    assert {h.document_id for h in hits} == {_DOC_A, _DOC_B}


def test_reindex_skips_docs_without_a_stored_split(stack, make_document,
                                                   stub_embeddings):
    make_document(_DOC_A, stack.corpus / "a.pdf")
    make_document(_DOC_B, stack.corpus / "b.pdf", with_vectors=False)  # parse only

    result = reindex(store=stack.store)

    assert result.documents == 1
    assert result.skipped == [_DOC_B]


def test_reindex_is_namespace_scoped(stack, make_document, stub_embeddings):
    make_document(_DOC_A, stack.corpus / "a.pdf")
    make_document(_DOC_B, stack.corpus / "b.pdf", namespace="tenant-a")

    result = reindex(store=stack.store, namespace="tenant-a")

    assert result.documents == 1
    hits = stack.store.query(vec(1.0), top_k=5, namespace="tenant-a")
    assert {h.document_id for h in hits} == {_DOC_B}


def test_reindex_is_idempotent(stack, make_document, stub_embeddings):
    make_document(_DOC_A, stack.corpus / "a.pdf")
    reindex(store=stack.store)
    result = reindex(store=stack.store)  # again — upserts overwrite
    assert result.documents == 1
    hits = stack.store.query(vec(1.0), top_k=10)
    assert len([h for h in hits if h.document_id == _DOC_A]) == 1, "no duplicates"


def test_empty_registry_reindexes_nothing(stack, stub_embeddings):
    result = reindex(store=stack.store)
    assert result.documents == 0 and result.chunks == 0
