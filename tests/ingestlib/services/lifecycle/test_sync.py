...
async def test_repair_deletes_the_older_duplicate(pipeline):
    ...
    # resurrect the crashed state IN THE REGISTRY: a replace that died before it
    # tombstoned v1 leaves two LIVE rows claiming one path (v1 older, v2 newer).
    ...
    with session_scope() as s:
        v1 = s.get(Document, first.doc_id)
        v1.status = "ingested"
        v1.created_at = datetime(2000, 1, 1, tzinfo=timezone.utc)
        s.get(Document, second.doc_id).created_at = datetime(2026, 1, 1, tzinfo=timezone.utc)
