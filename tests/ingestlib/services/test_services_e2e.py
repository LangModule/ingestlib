...
_CORE_STAGES = ("parse", "classify", "split", "embed", "upsert")


def test_ingest_completes_all_stages(ingested):
    assert ingested.status == "ingested"
    assert ingested.vectors == ingested.chunks > 0
    # the core five always run; a collection with auto_extract appends "extract"
    assert set(_CORE_STAGES) <= set(ingested.durations)


def test_on_stage_reported_every_stage_in_order(ingested):
    core_events = [(s, e) for s, e in _STAGE_EVENTS if s in _CORE_STAGES]
    assert core_events == [
        (stage, event) for stage in _CORE_STAGES for event in ("start", "done")
    ]
