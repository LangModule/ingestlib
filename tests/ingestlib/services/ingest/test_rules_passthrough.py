...
class _FakeStore(VectorStore):
    ...
    def count_vectors(self, document_id, namespace=""):
        return 0
...
def piped(monkeypatch, tmp_path):
    ...
    # Persistence is out of scope here (this checks arg passthrough only), so stub
    # both stores so the test runs offline — no blob writes, no registry/Postgres.
    monkeypatch.setattr(ingestor.artifacts, "save_parse", lambda *a, **k: None)
    monkeypatch.setattr(ingestor.artifacts, "find_by_path", lambda *a, **k: None)
    for name in ("save_parse", "save_classify", "save_split", "save_embed",
                 "save_status", "sync_collections"):
        monkeypatch.setattr(ingestor.registry, name, lambda *a, **k: None)
