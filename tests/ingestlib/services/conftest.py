...
def _truncate_registry() -> None:
    """Clear every document (children cascade) for a per-test clean slate."""
...
def stack(tmp_path, monkeypatch):
    """Local artifacts + sqlite vectors + a fresh registry, plus a corpus dir.

    The corpus lifecycle now reads the Postgres registry, so these tests need it
    (opt-in via RUN_REGISTRY_E2E=1 with the `registry` compose service up).
    """
    if os.environ.get("RUN_REGISTRY_E2E") != "1":
        pytest.skip("corpus lifecycle reads the registry — set RUN_REGISTRY_E2E=1")
...
def make_document(stack):
    ...
    def _make(
        ...
        *,
        ...
        source = Path(source)
        parse = synthetic_parse_result(doc_id, source)
        artifacts.save_parse(parse)
        registry.save_parse(doc_id, parse, namespace=namespace, source_path=source)
        if not with_vectors:
            return
        chunk = Chunk(
            chunk_id=0, section="body", heading="h", text="hello",
        ...
        split = SplitResult(
            sections=[Section(name="body", pages=[1], chunks=[chunk])], pages_used=1,
        )
        registry.save_split(doc_id, split)
        stack.store.upsert_chunks(
            doc_id, [chunk], [vec(1.0)], category="report", namespace=namespace
        )
        registry.save_embed(
            doc_id, vector_store="SqliteStore", vector_dim=DIM, vector_count=1,
