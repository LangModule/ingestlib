"""Corpus CLI (ingest/sync/list/show/collections/recollect/verify/remove/reindex)
driven through main().
...
def cli_stack(tmp_path, monkeypatch):
    """Local artifacts + sqlite + a fresh registry; model boundaries stubbed.

    default_store() is called fresh inside the commands, so it picks up this
    config. The corpus CLI now reads the Postgres registry (opt-in via
    RUN_REGISTRY_E2E=1 with the `registry` compose service up).
    """
    if os.environ.get("RUN_REGISTRY_E2E") != "1":
        pytest.skip("corpus CLI reads the registry — set RUN_REGISTRY_E2E=1")
    ...
    # reindex re-embeds through the ingestor's _embed_chunks → the same
...
def test_reindex_rebuilds(cli_stack, capsys):
    doc = cli_stack / "report.pdf"
    doc.write_bytes(b"content")
    main(["ingest", str(doc)])
    capsys.readouterr()
    ...
    assert "reindexed 1 document(s)" in capsys.readouterr().out


def test_show_prints_document_details(cli_stack, capsys):
    doc = cli_stack / "report.pdf"
    doc.write_bytes(b"content")
    main(["ingest", str(doc)])
    capsys.readouterr()

    assert main(["show", str(doc)]) == 0
    out = capsys.readouterr().out
    assert "report.pdf" in out and "category   report" in out


def test_collections_lists_counts(cli_stack, capsys):
    doc = cli_stack / "report.pdf"
    doc.write_bytes(b"content")
    main(["ingest", str(doc)])
    capsys.readouterr()
    ...
    out = capsys.readouterr().out
    assert "report" in out and "1 collection(s)" in out


def test_verify_reports_durable(cli_stack, capsys):
    doc = cli_stack / "report.pdf"
    doc.write_bytes(b"content")
    main(["ingest", str(doc)])
    capsys.readouterr()
    ...
    assert "durable across all stores" in capsys.readouterr().out


def test_recollect_reclassifies(cli_stack, capsys):
    doc = cli_stack / "report.pdf"
    doc.write_bytes(b"content")
    main(["ingest", str(doc)])
    capsys.readouterr()

    # the stub always classifies "report", so nothing moves — but the corpus
    # is re-classified and the command reports cleanly
    assert main(["recollect"]) == 0
    assert "recollected 1 document(s)" in capsys.readouterr().out
