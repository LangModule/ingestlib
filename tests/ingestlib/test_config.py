...
def test_reset_config_disposes_the_registry_engine(scratch_config, monkeypatch):
    """The registry DB engine (INGESTLIB_REGISTRY_URL) must reset like every other
    client — otherwise a changed registry URL keeps hitting the old database."""
    from ingestlib_registry import db as registry_db
    ...
    fake = _FakeEngine()
    monkeypatch.setattr(registry_db, "_engine", fake)
    monkeypatch.setattr(registry_db, "_session_factory", object())
    reset_config()
    assert fake.disposed, "reset must dispose the registry engine's pool"
    assert registry_db._engine is None, "reset must drop the registry engine cache"
