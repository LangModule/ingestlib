"""JSON Schema → Pydantic model, so an extract shape can be declared as data.

`extract()` takes a Pydantic class. An agent over MCP passes JSON; a collection
rule in rules.yaml carries an `extract_schema` block. Both turn that JSON Schema
into a dynamic Pydantic model the existing `extract()` accepts — the whole reason
extract works from data with no change to the library core. Handles scalars,
arrays (with item type), and nested objects (recursively); every field's
`description` flows into the model so it reaches extract's prompt.
"""
