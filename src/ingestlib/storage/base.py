...
sqlite, pgvector, mongodb, milvus, opensearch, weaviate) subclasses
VectorStore and handles its backend's quirks — ID schemes, metadata
encoding, deletion semantics — behind these three methods, so pipelines
are written once and run against whichever database is configured.
